    ...
    if (options.waitFor) {
      const waitForSelector = typeof options.waitFor === 'string' ? options.waitFor : options.selector
        try {
          ...
          return {
            pass: false,
            message: () => this.context.isNot ? `expected page to no longer have elements matching "${waitForSelector}" after waiting for"` : `expected page to contain element matching "${waitForSelector}" after waiting for"`
          }
        }
    }
